...
for i in opt_vals:
	...
	try:
		if i in [1,5]:
			...
			if i in opt_vals_lyapunov:
				best_lyapunov_func_vals = np.loadtxt(file_name)[:,2]
	...
	ax4.loglog((np.arange(nb_epoch)+1), \
            ...
            label=labels_dict[i], color=colors[color_count],
            linewidth=my_line_width, marker=my_markers[i-1])
	ax5.loglog((np.arange(nb_epoch)+1),
            (best_train_objective_vals[:nb_epoch] -
             min_fun_val)/(best_train_objective_vals[0]-min_fun_val),
            label=labels_dict[i], color=colors[color_count],
            linewidth=my_line_width, marker=my_markers[i-1])
	...
	if i in opt_vals_lyapunov:
		ax6.loglog((np.arange(nb_epoch)+1),(best_lyapunov_func_vals[:nb_epoch]),\
			label=labels_dict[i],color=colors[color_count],\
			linewidth=my_line_width,marker=my_markers[i-1])
		ax7.loglog(np.cumsum(best_time_vals[:nb_epoch]), (best_lyapunov_func_vals[:nb_epoch]),
                    label=labels_dict[i], color=colors[color_count],
                    linewidth=my_line_width, marker=my_markers[i-1])
	color_count +=1
...
ax5.set_ylabel('Suboptimality (log scale)', fontsize=legend_font_size)

do_tight_layout_for_fig(fig5)
fig5.savefig(figure_name1+'_suboptimality_1.png', dpi=fig5.dpi)
fig5.savefig(figure_name1+'_suboptimality_1.pdf', dpi=fig5.dpi)


ax6.set_xlabel('Iterations (log scale)', fontsize=legend_font_size)
ax6.set_ylabel('Lyapunov f.v. (log scale)', fontsize=legend_font_size)

do_tight_layout_for_fig(fig6)
fig6.savefig(figure_name1+'_lyapunov_.png', dpi=fig6.dpi)
fig6.savefig(figure_name1+'_lyapunov_.pdf', dpi=fig6.dpi)

ax7.set_xlabel('Time (log scale)', fontsize=legend_font_size)
ax7.set_ylabel('Lyapunov f.v. (log scale)', fontsize=legend_font_size)

do_tight_layout_for_fig(fig7)
fig7.savefig(figure_name1+'_lyapunov_time_.png', dpi=fig7.dpi)
fig7.savefig(figure_name1+'_lyapunov_time_.pdf', dpi=fig7.dpi)
