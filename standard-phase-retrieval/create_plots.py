...
if fun_num == 0:
	...
	files = {
		...
		6: 'results/gd_bt_global_1_abs_fun_num_3.txt',
		7: 'results/gd_bt_1_abs_fun_num_3.txt',
		8: 'results/ibgm_1_abs_fun_num_3.txt',
	}
if fun_num == 1:
	files = {
		...
		6: 'results/gd_bt_global_2_abs_fun_num_4.txt',
		7: 'results/gd_bt_2_abs_fun_num_4.txt',
		8: 'results/ibgm_2_abs_fun_num_4.txt',
	}
...
nb_epoch = 1000
opt_vals = np.array([2, 3, 4, 6, 7, 8]) #ignoring cocain bpg

opt_vals_lyapunov = np.array([3,6])
color_count = 0
...
for i in opt_vals:
	...
	if i in [1,5]:
		ax3.loglog((np.arange(nb_epoch)+1), (best_gamma_est_vals[:nb_epoch]),
				...
				linewidth=my_line_width, marker=my_markers[i-1])
	if i in opt_vals_lyapunov:
		ax5.loglog((np.arange(nb_epoch)+1),(best_lyapunov_func_vals[:nb_epoch]),\
				label=labels_dict[i],color=colors[color_count],\
				linewidth=my_line_width,marker=my_markers[i-1])
		ax6.loglog(np.cumsum(best_time_vals[:nb_epoch]), \
				(best_lyapunov_func_vals[:nb_epoch]),\
				label=labels_dict[i],color=colors[color_count],\
				linewidth=my_line_width,marker=my_markers[i-1])
...
ax5.set_ylabel('Lyapunov f.v. (log scale)',fontsize=legend_font_size)

do_tight_layout_for_fig(fig5)
fig5.savefig(figure_name1+'_lyapunov_.png', dpi=fig5.dpi)
fig5.savefig(figure_name1+'_lyapunov_.pdf', dpi=fig5.dpi)


ax6.set_xlabel('Time (log scale)',fontsize=legend_font_size)
ax6.set_ylabel('Lyapunov f.v. (log scale)',fontsize=legend_font_size)

do_tight_layout_for_fig(fig6)
fig6.savefig(figure_name1+'_lyapunov_time_.png', dpi=fig6.dpi)
fig6.savefig(figure_name1+'_lyapunov_time_.pdf', dpi=fig6.dpi)
