...
if fun_num == 3:
	# fun_num = 3 for No Regularization
	del_val = 0.15
	eps_val = 0.00001
	uL_est =  10
	lL_est = 1e-4*uL_est
...
def make_update(y, grad, uL_est):
	...
	temp_y = np.maximum(y/(1 + (y*grad + lam*y)*(1/uL_est)),1e-5)
...
def make_update1(y, grad, uL_est):
	...
	tau = (1/uL_est)
	temp_y = np.maximum((np.sqrt((1 + (y*grad + lam*y)*tau)**2 +
                           4*lam*tau*y*y) - (1 + (y*grad + lam*y)*tau))/(2*lam*tau*y), 1e-5)

	return temp_y

def make_update2(y, grad, uL_est):
	# Bregman Proximal Mapping with No Regularization


	temp_y = np.maximum(y/(1 + (y*grad)*(1/uL_est)),1e-5)
...
def do_ub_search(A, b, y_U, uL_est):
	...
	if fun_num == 1:
		...
		x_U = make_update2(y_U, grad_u, uL_est)
	else:
		raise
...
def obtain_delta(A, b, y_U, uL_est):
	...
	if fun_num == 1:
		...
		tx_U = make_update2(y_U, grad_u, uL_est)
	else:
		raise
